...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.delete("/api/admin/tournaments/:id", async (req, res) => {
    ...
    try {
      ...
      await pool.query("DELETE FROM tournament_chats WHERE tournament_id=$1", [req.params.id]);
      await pool.query("DELETE FROM prize_distributions WHERE tournament_id=$1", [req.params.id]);
      await pool.query("DELETE FROM tournament_rewards WHERE tournament_id=$1", [req.params.id]);
